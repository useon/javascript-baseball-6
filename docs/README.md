...
- [x] 게임을 재시작 하는 경우 컴퓨터 숫자를 뽑는 순서로 돌아가는 기능
- [x] 게임을 종료하는 경우 종료 안내 문구를 출력하고 종료하는 기능

## 테스트코드구현목록

- [x] Validator 객체 테스트 구현
  - 사용자의 1-9 범위 서로 다른 세 개의 숫자 입력값의 유효성 검증이 올바르게 되는지 확인한다.
  - 사용자의 재시작 여부에 대한 입력값의 유효성 검증이 올바르게 되는지 확인한다.
- [x] Judge 클래스 테스트 구현
  - 같은 수가 같은 자리에 있으면 스트라이크, 다른 자리에 있으면 볼, 같은 수가 전혀 없으면 낫싱이 올바르게 판단되는지 확인한다.
